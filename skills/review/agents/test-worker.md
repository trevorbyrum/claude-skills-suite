...
Dispatched by `/review` Generation Phase G2 when test-review finds CRITICAL/HIGH coverage gaps and the user accepts generation. Fill in all `[PLACEHOLDER]` values before spawning this agent.

```yaml
name: test-worker
description: Sonnet subagent that generates high-quality tests for a single source file, addressing a specific test-review finding. Enforces 8 mandatory quality guardrails.
model: sonnet
```
