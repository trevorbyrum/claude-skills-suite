...
First define a portable timeout wrapper (run this once before the snippets below):
```bash
# gtimeout on macOS (coreutils), timeout on Linux/GNU; no-op wrapper if neither is installed.
...
**JS/TS primary:**
```bash
run_to 120 npx dependency-cruiser --output-type json --no-config . \
  > /tmp/ia-depcruiser.json 2>/tmp/ia-depcruiser.err \
  || run_to 90 npx madge --json . > /tmp/ia-madge.json 2>/tmp/ia-madge.err
```

**Python primary:**
```bash
run_to 90 python -m vulture . --min-confidence 60 > /tmp/ia-vulture.txt 2>&1 || true
...
**Go primary:**
```bash
run_to 60 go list -json ./... > /tmp/ia-go-list.json 2>/tmp/ia-go-list.err
run_to 60 go vet ./... > /tmp/ia-go-vet.txt 2>&1 || true
```

**Rust primary:**
```bash
run_to 120 cargo udeps --output json > /tmp/ia-cargo-udeps.json 2>&1 || true
run_to 60 cargo tree --format "{p} {d}" > /tmp/ia-cargo-tree.txt 2>&1 || true
```
...
|---|---|---|
| JS/TS | knip | `run_to 120 npx knip --reporter json > /tmp/ia-knip.json 2>&1` |
...
```
/init (pivot mode) Sonnet subagent invokes this after context rewrite.
