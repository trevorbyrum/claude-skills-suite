...
Reference for `/init` pivot pathway. When the project direction changes enough that the current `project-context.md` and `project-plan.md` are no longer the truth, this orchestrates an adversarial-triaged rewrite + safe incremental removal of old-direction code in waves.

Use when the change is too big for `/save` doc-refresh (heavy mode) and too structural for a `/build-plan` update — i.e., "the direction is different now."
...
Pivot mode automates the analysis, structures the decisions, and executes removal
...
and runs `scripts/analyze-deps.sh`. It runs:
...
Dispatch both subagents in parallel (single tool-use block with two `Agent` calls — the runtime parallelizes them):

1. **Sonnet subagent "cut" side**: Spawn an `Agent` (`subagent_type: "general-purpose"`) with the `agents/adversarial-debate.md` "candidate-challenge / steelman cut" framing applied to the prompt. Output → `/tmp/pivot-debate-cut.md`.

2. **Sonnet subagent "keep" side**: Spawn another `Agent` with the "steelman keep" framing applied to the same prompt. Output → `/tmp/pivot-debate-keep.md`.

**Fallback**:
- One subagent fails or times out → spawn a single judge subagent with the surviving partial output and ask for the strongest case + caveats.
- Both fail → surface to user; pivot pauses.

After both return, merge disagreements into the candidate list:
...
### Phase 4.5: Adversarial Challenge II [S, Two Sonnet subagents]
...
Same dispatch pattern as Phase 3.5 — two parallel Sonnet subagents (one steelmanning "the triage is too aggressive," one steelmanning "the triage is too cautious"). See `agents/adversarial-debate.md` mode `triage-challenge`. Same fallback chain.
...
1. Runs drift-review inline against the *updated* docs (doc-to-code and code-to-doc comparison)
2. Runs completeness-review inline (scans for stubs, TODOs, placeholders, empty handlers)
...
### Phase 8: Full Project Review [/review]

The codebase has been surgically altered — run a full multi-lens review to catch
anything the wave execution or verification missed.

Tell the user:

> "Pivot execution and verification complete. Running /review on the
> surviving codebase to catch quality issues introduced during removal."

Invoke `/review`. This dispatches the full review suite (12 Sonnet lenses in parallel) against the post-pivot codebase. The review reads the *updated* project docs from Phase 2, so it evaluates against the new direction.

If `/review` surfaces findings:
- **CRITICAL/HIGH**: Present to user. These should be resolved before the final
  doc update. Offer to run `/execute fix` to dispatch fixes.
- **MEDIUM/LOW**: Log in pivot-summary.md for follow-up. Don't block the pivot.

Append review summary to pivot-summary.md.

**Exit condition**: `/review` has run. Critical/high findings resolved or
...
This reference inherits the cross-cutting rules from `/init`'s footer (`../../references/cross-cutting-rules.md`). Agent prompt paths (`agents/adversarial-debate.md`, `agents/deep-analysis.md`, `agents/doc-update.md`, `agents/project-review.md`, `agents/verification.md`, `agents/wave-executor.md`) are relative to `skills/init/` and were ported alongside this reference.
