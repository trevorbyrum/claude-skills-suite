...
Reference for `/init` sub-project mode. Invoked when a completed sub-project needs to merge back into its parent — research artifacts, documents, artifact DB records, source files, and all. Generates a merge-specific plan at runtime, presents it for approval, then executes in safety-tiered batches so nothing is lost or overwritten without confirmation.

**When invoked**: the user is in `/init` and asks to merge an existing sub-project back into its parent, OR runs `/init merge <sub-project-path-or-name>`.

This is a reference, not a top-level skill — `/init` reads this file when sub-project-mode detection routes here.
...
2. For each record in sub-project DB:
   - Prefixes the `skill` field with `sub:<name>/` (e.g., `research-execute` → `sub:auth-service/research`)
...
```
User: "/init merge frontend-redesign --keep"
...
This reference inherits the cross-cutting rules from `/init`'s footer (`../../references/cross-cutting-rules.md`). All agent prompt paths above are relative to `skills/init/` — `agents/scanner.md`, `agents/db-merger.md`, `agents/doc-merger.md` were ported alongside this reference.
