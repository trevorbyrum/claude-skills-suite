...
# Bootstrap
# ---------------------------------------------------------------------------
# Resolve a portable per-tool timeout wrapper: honor $GTIMEOUT if it points at a real binary, else
# gtimeout (macOS via coreutils), else timeout (Linux/GNU). If none exists, run without a wrapper
# rather than calling a missing absolute path (which used to break every Linux user).
_TIMEOUT_BIN=""
if [ -n "${GTIMEOUT:-}" ] && command -v "$GTIMEOUT" >/dev/null 2>&1; then
...
run_to() { if [ -n "$_TIMEOUT_BIN" ]; then "$_TIMEOUT_BIN" "$TIMEOUT_SECS" "$@"; else "$@"; fi; }
TIMEOUT_SECS=120
PROJECT_DIR="$(pwd)"
