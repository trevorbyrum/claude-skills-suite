...
1. Read the impact-analysis skill at:
   [PROJECT_PATH]/skills/init/references/impact-analysis.md

2. Follow impact-analysis completely. Run all 4 modes:
...
3. Run dead code detection using the helper script:
   bash [PROJECT_PATH]/skills/init/scripts/analyze-deps.sh \
     --project-dir [PROJECT_PATH]

4. Run doc-code diff: compare project-context.md + features.md + project-plan.md
   against the actual codebase. Items in code but NOT in docs = removal candidates.

5. Merge all sources. Deduplicate candidates. Assign confidence scores:
   - High: 2+ sources agree (e.g., dead code tool + doc-code diff)
   - Medium: 1 source, static analysis
   - Low: heuristic only (grep-based)

6. Write results to these temp files (NOT to the artifact DB):
...
7. When complete, report back with:
